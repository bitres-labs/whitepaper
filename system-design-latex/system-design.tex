...
\author{Bitres Labs\\\url{https://github.com/bitres-labs}}
\date{\today}
...
\begin{itemize}
  ...
  \item \textbf{Interest Rate Control Module}: Updates the annualized yield rates for BTD and BTB staking, following the federal funds rate and secondary market price respectively;
...
\subsubsection{Oracle Parameter Solidification Strategy}
To prevent price manipulation, oracle configuration is fully solidified at deployment:
\begin{itemize}
  \item \textbf{Immutable Parameters}: Pyth price ID, Redstone data source ID, precision configuration, TWAP enable flag, etc., are never changeable after being set in the constructor.
...
\begin{itemize}
  \item The annualized rate is updated by the interest rate feed or directly by governance, defaulting to reference the upper limit of the US federal funds target range;
  \item Interest accumulates per second and is paid to stakers through BTD minting;
  \item The operator can levy an additional fee rate on interest income (default 10\%, adjustable by governance).
\end{itemize}
...
\textbf{Adjustment Magnitude Algorithm:} When rate adjustment is needed, the adjustment magnitude equals the daily price change of BTB (expressed in basis points). Specifically:
\begin{itemize}
  \item When price $<0.99$ and falling, rate increase magnitude = daily decline;
  \item When price $>1.01$ and rising, rate decrease magnitude = daily increase.
\end{itemize}
...
\begin{itemize}
  \item \textbf{BTD Rate:} Synchronized with the federal funds rate, with risk premium added if necessary, adjustable by community governance; additionally, when depositors collect interest, an extra 10\% BTD is minted as system fee and deposited into the treasury;
  \item \textbf{BTB Rate:} Dynamically adjusted through price intervals, encouraging buying when bonds are at discount and exiting when at premium;
...
\begin{itemize}
  \item \textbf{Core Constants Fully Solidified} (immutable):
    \begin{itemize}
      \item All core contract addresses (BTD, BTB, BRS, Treasury, Minter, Config, etc.)
      \item All oracle configurations (Pyth/Redstone/Chainlink feed IDs, data source addresses, precision)
      \item IUSD inflation parameters (2\% annual, monthly growth factor)
      \item Core trading pool addresses (BTD/USDC, BTB/BTD, BRS/BTD, WBTC/USDC)
...
A very small number of parameters retain governance capability, but are strictly constrained by \textbf{four security mechanisms}:

\textbf{1. Price Deviation Tolerance} (default 1\%)
\begin{itemize}
...
\textbf{2. BTD Minting Fee} (default 1\%)
\begin{itemize}
  \item \textit{Range Limit}: 0\%--2\%
  \item \textit{Cooldown Period}: At least 3 days between adjustments
\end{itemize}

\textbf{3. BTB Minimum Price Protection} (default 0.5 BTD)
\begin{itemize}
  \item \textit{Range Limit}: 0.3--0.7 BTD
  \item \textit{Cooldown Period}: At least 7 days between adjustments
\end{itemize}

\textbf{4. BTB Maximum Annual Rate} (default 20\%)
\begin{itemize}
  \item \textit{Range Limit}: 10\%--50\%
  \item \textit{Cooldown Period}: At least 7 days between adjustments
\end{itemize}

\textbf{5. Mining Allocation Weights}
\begin{itemize}
...
\textbf{6. Emergency Operations} (contract pause, blacklist)
\begin{itemize}
...
\begin{itemize}
  \item \textbf{47\% of parameters fully solidified}: Core addresses, oracle configurations, IUSD inflation rate, BRS total supply, etc., set as immutable at deployment, never changeable
  \item \textbf{20\% are static constants}: Mathematical precision, time conversion, safety ceilings, etc., determined at compile time
  \item \textbf{Only 33\% of parameters are governable}: A very small number of parameters like fees, deviation thresholds, rate caps retain governance capability, but are strictly constrained by four security mechanisms (boundary checks, cooldown periods, whitelist restrictions, event logs)
...
\subsubsection{Interest Rate Interval Constants (InterestPool)}
...
\midrule
\endhead
LOWER\_PRICE\_THRESHOLD & 0.99 BTD & BTB price lower threshold \\
UPPER\_PRICE\_THRESHOLD & 1.01 BTD & BTB price upper threshold \\
CR\_MIN & 30\% & CR minimum threshold \\
CR\_LOW & 90\% & CR low threshold \\
CR\_HIGH & 120\% & CR high threshold \\
CR\_MAX & 200\% & CR maximum threshold \\
BTD\_BASE\_RATE & 400 bps & BTD base rate (4\%) \\
BTD\_MAX\_RATE & 2000 bps & BTD maximum rate (20\%) \\
BTD\_MIN\_RATE & 0 bps & BTD minimum rate (0\%) \\
\bottomrule
\end{longtable}
\end{center}
}
...
PRICE\_ORACLE & Price oracle contract address \\
IDEAL\_USD\_MANAGER & IUSD manager address \\
INTEREST\_POOL & Interest pool contract address \\
STAKING\_ROUTER & Staking router contract address \\
FARMING\_POOL & Mining pool contract address \\
ST\_BTD & stBTD vault address \\
ST\_BTB & stBTB vault address \\
VESTING\_VAULT & Vesting vault address \\
GOVERNOR & Governance contract address \\
TWAP\_ORACLE & TWAP oracle address \\
\bottomrule
\end{longtable}
\end{center}

\subsubsection{Price Oracle Data Source Addresses (ConfigCore)}
...
\midrule
\endhead
CHAINLINK\_BTC\_USD & Chainlink BTC/USD price feed \\
CHAINLINK\_WBTC\_BTC & Chainlink WBTC/BTC price feed \\
PYTH\_WBTC & Pyth WBTC price feed \\
REDSTONE\_WBTC & Redstone WBTC price feed \\
\bottomrule
\end{longtable}
\end{center}

\subsubsection{Uniswap V2 Trading Pair Addresses (ConfigCore)}
...
\midrule
\endhead
POOL\_WBTC\_USDC & WBTC-USDC trading pair (for WBTC price) \\
POOL\_BTD\_USDC & BTD-USDC trading pair (for BTD price) \\
POOL\_BTB\_BTD & BTB-BTD trading pair (for BTB price) \\
POOL\_BRS\_BTD & BRS-BTD trading pair (for BRS price) \\
\bottomrule
\end{longtable}
\end{center}

\subsubsection{Oracle Configuration Parameters (PriceOracle)}
...
\subsubsection{Vesting Parameters (VestingVault)}
...
\midrule
\endhead
MINT\_FEE\_BP & 100 bps & 0--200 bps & Minting fee (1\%) \\
REDEEM\_FEE\_BP & 0 bps & 0--200 bps & Redemption fee \\
INTEREST\_FEE\_BP & 1000 bps & 0--2000 bps & Interest fee (10\%) \\
\bottomrule
\end{longtable}
\end{center}
...
\midrule
\endhead
MIN\_BTB\_PRICE & 0.5 BTD & 0.3--0.7 BTD & BTB minimum price protection \\
MAX\_BTB\_RATE & 2000 bps & 1000--5000 bps & BTB maximum rate (20\%) \\
PCE\_MAX\_DEVIATION & 2\% & 0--10\% & PCE maximum deviation rate \\
\bottomrule
\end{longtable}
\end{center}
}

\subsubsection{Address Parameters (ConfigGov)}
...
\midrule
\endhead
PCE\_FEED & Chainlink PCE oracle address \\
\bottomrule
\end{longtable}
\end{center}
