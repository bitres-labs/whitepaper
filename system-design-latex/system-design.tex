...
\title{Bitres System Design Document: Functional Architecture, Weak Governance, and Progressive Decentralization}
\author{Invariant Core\\
\texttt{invariantcore@gmail.com}\\
\url{https://www.bitres.org}}
\date{\today}

\renewcommand{\shorttitle}{Bitres System Design Document: Functional Architecture, Weak Governance, and Progressive Decentralization}
\hypersetup{
  colorlinks=true,
  linkcolor=blue,
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Bitres System Design Document: Functional Architecture, Weak Governance, and Progressive Decentralization},
...
\subsection{Design Philosophy: Functional, Weak Governance, and Progressive Decentralization}
The system adopts a design philosophy of \textbf{functional programming}, \textbf{weak governance}, and \textbf{progressive decentralization}, aiming to achieve immutability and predictability similar to Bitcoin while providing a safe path from initial deployment to full community control:
\begin{itemize}
  ...
  \item \textbf{Weak Governance Principle}: The system hard-codes the vast majority of parameters as immutable constants at launch, retaining only minimal governance capability for a very small number of necessary parameters. Governable parameters must satisfy strict security mechanisms: boundary checks, cooldown periods, whitelist restrictions, and event logging.
  \item \textbf{Progressive Decentralization}: To allow post-launch bug fixes and iterative improvements, all core functional contracts adopt the UUPS (Universal Upgradeable Proxy Standard) proxy pattern, enabling upgrades without disrupting user-facing addresses. Governance authority transitions through three phases: (1) administrator full control at launch for rapid bug fixing; (2) transfer to on-chain governance (BRS token voting + timelock) after system stabilization; (3) permanent freeze via ownership renunciation for battle-tested contracts that no longer need changes.
  \item \textbf{Parameter Classification System}:
    \begin{itemize}
      ...
      \item \textit{Deployment-Configured Parameters} (initialized once): Core addresses, oracle IDs, inflation parameters, etc., set during contract initialization and never modified thereafter
    ...
    \end{itemize}
\end{itemize}
This design ensures the system behaves as close to Bitcoin's immutability as possible while retaining minimal flexibility to handle extreme situations and providing a credible path toward full decentralization.
...
\begin{itemize}
  \item \textbf{Static Constants}: The annual inflation rate (2\% = 0.02) and monthly growth factor ($1.02^{1/12} \approx 1.001651581301920174$) are defined as static constants in the Constants library, compiled into bytecode and never modifiable.
  \item \textbf{PCE Oracle Configuration}: The PCE data source address is configured through ConfigGov (governable to allow feed upgrades), while precision configuration is static.
...
\begin{itemize}
  \item \textbf{Governable External Addresses}: External oracle addresses (Chainlink feeds, Pyth) and DEX pool addresses are governable to allow upgrades when external systems change. Changes are subject to timelock delay (7 days) and whitelist restrictions.
  ...
  \item \textbf{Functional Price Aggregation}: Price calculation logic is encapsulated as a pure function library, including dual-source consistency validation, deviation checking, etc., independent of mutable state, ensuring the algorithm is transparent and immutable.
\end{itemize}
...
The Bitres system adopts a \textbf{progressive decentralization} model for governance. Because newly deployed smart contracts may contain undiscovered bugs, the system launches with administrator control to enable rapid iteration and bug fixing, then gradually transitions to community governance as the protocol matures and gains confidence. The governance architecture follows the OpenZeppelin Governor standard, providing transparent and auditable proposal and voting processes.

\subsection{Progressive Decentralization Lifecycle}

The system transitions through three distinct governance phases:

\subsubsection{Phase 1: Administrator Mode (Launch)}
At system genesis, all upgradeable contracts are owned by the deployer (a multi-signature wallet). This enables:
\begin{itemize}
  \item Rapid bug fixes and security patches without governance delay
  \item Parameter tuning based on real market conditions
  \item Contract upgrades via \texttt{upgradeToAndCall()} to deploy improved implementations
  \item Quick response to security incidents (emergency pause, parameter adjustment)
\end{itemize}
During this phase, the administrator has full authority to upgrade contracts and adjust all governable parameters. All operations are recorded in on-chain events for transparency.

\subsubsection{Phase 2: Governance Mode (After Stabilization)}
Once the system has operated stably for a sufficient period and undergone thorough auditing, the administrator transfers ownership of all upgradeable contracts to a \texttt{TimelockController}:
\begin{itemize}
  \item The \texttt{TimelockController} is controlled by the \texttt{Governor} contract (BRS token voting)
  \item All parameter changes and contract upgrades require: proposal creation $\rightarrow$ voting period (7 days) $\rightarrow$ timelock delay (2 days) $\rightarrow$ execution
  \item Community members holding sufficient BRS can create proposals and vote
  \item Emergency operations (contract pause) can use expedited governance processes
\end{itemize}
The transition is executed by calling \texttt{transferOwnership(timelockAddress)} on each upgradeable contract, a one-way operation that permanently removes administrator privileges.

\subsubsection{Phase 3: Selective Freeze (Per-Contract)}
For contracts that have been battle-tested and are not expected to require further changes, the governance community can propose to permanently freeze them:
\begin{itemize}
  \item A governance proposal calls \texttt{renounceOwnership()} on the target contract
  \item Once \texttt{owner() == address(0)}, the \texttt{onlyOwner} modifier always reverts
  \item The \texttt{\_authorizeUpgrade()} function can never be called, making the contract permanently immutable
  \item \textbf{This operation is irreversible} --- only recommended for thoroughly audited, stable contracts
  \item Contracts and parameters that still need adjustment retain governance control indefinitely
\end{itemize}

\textbf{Classification of Freeze Candidates:}
\begin{center}
\begin{tabular}{lll}
\toprule
Category & Contracts & Freeze Likelihood \\
\midrule
Early freeze candidates & ConfigGov, PriceOracle & High (pure config/oracle) \\
Medium-term candidates & Minter, Treasury & Medium (core business logic) \\
Long-term governance & InterestPool, FarmingPool & Low (needs ongoing tuning) \\
Retain governance & IdealUSDManager, Governor & Indefinite (evolving requirements) \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Contract Upgradeability Architecture}

The system uses the \textbf{UUPS (Universal Upgradeable Proxy Standard)} pattern from OpenZeppelin. Each upgradeable contract is deployed as a pair: an implementation contract containing the logic, and an ERC1967 proxy contract that stores state and delegates calls to the implementation.

\subsubsection{Upgradeable Contracts (8)}
\begin{center}
\begin{tabular}{ll}
\toprule
Contract & Role \\
\midrule
ConfigGov & Parameter governance \\
IdealUSDManager & IUSD inflation management \\
PriceOracle & Multi-source price aggregation \\
Treasury & Collateral custody and buyback \\
Minter & Minting and redemption core \\
InterestPool & Interest rate calculation and distribution \\
FarmingPool & BRS mining reward distribution \\
Governor & On-chain governance (proposal/voting) \\
\bottomrule
\end{tabular}
\end{center}

These contracts inherit \texttt{Initializable}, \texttt{UUPSUpgradeable}, and other OpenZeppelin Upgradeable mixins. Key properties:
\begin{itemize}
  \item \textbf{Transparent to users}: Proxy addresses are stable; upgrades only change the underlying implementation
  \item \textbf{State preservation}: All storage (balances, configurations, accumulated values) survives upgrades
  \item \textbf{Authorization guard}: \texttt{\_authorizeUpgrade()} enforces \texttt{onlyOwner}, preventing unauthorized upgrades
  \item \textbf{Storage gap}: Each contract reserves \texttt{uint256[50] private \_\_gap} for future storage extensions
  \item \textbf{Initialization protection}: Constructors call \texttt{\_disableInitializers()} to prevent implementation contracts from being initialized directly
\end{itemize}

\subsubsection{Non-Upgradeable Contracts (7)}
\begin{center}
\begin{tabular}{lll}
\toprule
Contract & Role & Reason for Non-Upgradeability \\
\midrule
BTD, BTB & ERC20 tokens & Standard token, no business logic \\
BRS & Governance token & Fixed supply, ERC20Votes \\
stBTD, stBTB & ERC4626 vaults & Pure vault, no complex logic \\
ConfigCore & Address registry & Immutable system anchor \\
UniswapV2TWAPOracle & TWAP oracle & Stateless, replaceable via PriceOracle \\
\bottomrule
\end{tabular}
\end{center}

ConfigCore serves as the immutable anchor of the system: it stores all contract addresses (including proxy addresses of upgradeable contracts) as truly immutable values set at deployment. Since proxy addresses never change, ConfigCore's references remain valid even after upgrades to the underlying implementations.
...
\begin{itemize}
  \item \textbf{Core Constants Fully Solidified}:
    \begin{itemize}
      \item All internal contract addresses stored in ConfigCore (immutable --- set in constructor, compiled into bytecode)
      \item For upgradeable contracts, core references (e.g., ConfigCore address, ConfigGov address) are set once during proxy initialization and never modified by any function --- effectively immutable despite being stored in contract storage
      \item IUSD inflation parameters (2\% annual, monthly growth factor) --- static constants in library
      \item BRS total supply cap (2.1 billion) and halving mechanism --- static constants
      \item Core calculation algorithms and precision configurations --- pure function libraries
    \end{itemize}
...
\begin{itemize}
  \item Oracle addresses (Chainlink feeds, Pyth) --- subject to 7-day timelock and whitelist
...
\textbf{Protocol Upgrades (UUPS):}
\begin{itemize}
  \item Deploy new implementation contract and call \texttt{upgradeToAndCall()} on the proxy (must go through timelock in governance mode)
  \item All 8 upgradeable contracts (ConfigGov, IdealUSDManager, PriceOracle, Treasury, Minter, InterestPool, FarmingPool, Governor) support this mechanism
  \item Oracle contract address updates via ConfigGov
  \item Permanent freeze of mature contracts via \texttt{renounceOwnership()} governance proposal
\end{itemize}
...
\subsection{Core Advantages of Functional Architecture, Weak Governance, and Progressive Decentralization}

The system design follows three core principles to ensure long-term stability, predictability, and a credible path toward decentralization:
...
\begin{itemize}
  ...
  \item Gas efficiency: static constants are compiled into bytecode (3 gas read); for upgradeable contracts, initialized-once parameters use storage (100 gas warm read), an acceptable tradeoff for upgradeability
\end{itemize}

\textbf{Weak Governance Principle} anchors system behavior to Bitcoin-like immutability:
\begin{itemize}
  \item \textbf{Static constants}: Mathematical precision, time conversion, safety ceilings, interest rate parameters, etc., determined at compile time --- permanently immutable
  \item \textbf{Initialized-once parameters}: Core addresses (in ConfigCore: immutable; in upgradeable contracts: set during initialization and never modified) --- effectively immutable
  \item \textbf{Governable parameters}: A small number of parameters like external system addresses, fees, deviation thresholds, rate caps retain governance capability, but are strictly constrained by security mechanisms (boundary checks, cooldown periods, whitelist restrictions, event logs)
...
\textbf{Progressive Decentralization} provides a credible path from launch to full community control:
\begin{itemize}
  \item \textbf{UUPS Proxy Architecture}: 8 core contracts use proxy pattern, enabling seamless upgrades without changing user-facing addresses or disrupting stored state
  \item \textbf{Three-Phase Lifecycle}: Administrator control at launch $\rightarrow$ governance transfer after stabilization $\rightarrow$ permanent freeze for battle-tested contracts
  \item \textbf{Selective Immutability}: Not all contracts follow the same path --- mature, stable contracts can be permanently frozen while evolving components retain governance control
  \item \textbf{Non-Upgradeable Anchors}: Token contracts (BTD, BTB, BRS), vault contracts (stBTD, stBTB), and the address registry (ConfigCore) are non-upgradeable by design, serving as immutable trust anchors
\end{itemize}

\subsection{Design Objectives Achieved}

Through the combination of functional architecture, weak governance, and progressive decentralization, the system achieves:
\begin{itemize}
  ...
  \item \textbf{Security}: Attack vectors like price manipulation and inflation rate tampering are completely eliminated; upgrade authorization is strictly controlled
  \item \textbf{Minimal Trust}: Users need not trust that governors won't change core rules; mature contracts can be permanently frozen to remove all trust assumptions
  \item \textbf{Safe Iteration}: UUPS proxy pattern enables bug fixes and improvements without disrupting the system, critical during the early maturation period when undiscovered issues may surface
  \item \textbf{Credible Decentralization}: The three-phase lifecycle provides a transparent, verifiable path from centralized launch to community governance to permanent immutability
\end{itemize}

This design philosophy ensures the BRS system maintains the same level of stability and trustworthiness as the Bitcoin protocol while providing decentralized stablecoin services. The progressive decentralization model acknowledges that complex smart contract systems benefit from a stabilization period, while committing to an irreversible path toward community control. Through on-chain governance mechanisms, the community can democratically adjust necessary parameters and drive protocol evolution within strict constraints. Therefore, as long as implementations follow the mechanisms and parameters specified in this document, they can remain consistent with the whitepaper and expand into various implementation schemes in the future.
...
\begin{itemize}
  ...
  \item \textbf{Upgrade Safety}: During UUPS proxy upgrades, pure function library logic remains unchanged and deterministic, reducing upgrade risk --- only the contract layer (state management, permissions) changes
  \item \textbf{Gas Efficiency}: Inline functions avoid cross-contract calls; static constants compiled into bytecode at 3 gas read cost
...
\subsection{Initialized-Once Parameters}

This category includes parameters that are set once and never modified. There are two subtypes:

\begin{itemize}
  \item \textbf{True Immutables} (in non-upgradeable contracts like ConfigCore): Set in the constructor and compiled into bytecode. Read cost: 3 gas.
  \item \textbf{Proxy-Initialized} (in upgradeable contracts): Set during \texttt{initialize()} and stored in contract storage. No setter function exists to modify them after initialization. Read cost: 100 gas (warm) / 2,100 gas (cold). This is an acceptable tradeoff for the ability to upgrade contract logic via UUPS proxy.
\end{itemize}

\subsubsection{Core Token Addresses (ConfigCore --- True Immutable)}
...
\subsubsection{Core Contract Addresses (ConfigCore --- True Immutable)}
...
\midrule
\endhead
treasuryAddr & Treasury proxy address (UUPS) \\
minterAddr & Minting and redemption proxy address (UUPS) \\
priceOracleAddr & Price oracle proxy address (UUPS) \\
idealUsdManagerAddr & IUSD manager proxy address (UUPS) \\
interestPoolAddr & Interest pool proxy address (UUPS) \\
stakingRouterAddr & Staking router contract address \\
farmingPoolAddr & Mining pool proxy address (UUPS) \\
stBtdAddr & stBTD vault address \\
stBtbAddr & stBTB vault address \\
governorAddr & Governance proxy address (UUPS) \\
...
\textbf{Note:} For UUPS upgradeable contracts, ConfigCore stores the stable \textit{proxy} addresses. These addresses never change even when the underlying implementation is upgraded, ensuring all cross-contract references remain valid.

\subsubsection{Proxy-Initialized Parameters (Upgradeable Contracts)}

The following parameters are set once during \texttt{initialize()} in upgradeable contracts and have no setter functions:

\begin{center}
\begin{longtable}{p{4cm}p{3cm}p{7cm}}
\toprule
Parameter Name & Contract & Description \\
\midrule
\endhead
configCore & Treasury, Minter, InterestPool, FarmingPool, PriceOracle & ConfigCore address reference \\
configGov & Minter, InterestPool, PriceOracle, IdealUSDManager & ConfigGov proxy address reference \\
rewardToken & FarmingPool & BRS token address \\
startTime & FarmingPool & Mining start timestamp \\
\bottomrule
\end{longtable}
\end{center}
...
\midrule
Static Constants (constant) & $\sim$43 & $\sim$44\% \\
Initialized-Once: True Immutable (ConfigCore) & $\sim$18 & $\sim$18\% \\
Initialized-Once: Proxy-Initialized & $\sim$8 & $\sim$8\% \\
Governable Variables (governable) & $\sim$29 & $\sim$30\% \\
\midrule
\textbf{Total} & \textbf{$\sim$98} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{center}
...
Approximately 70\% of system parameters are solidified at deployment (constant + initialized-once), with only 30\% retaining governance capability. This ``weak governance'' design, combined with progressive decentralization, ensures:

\begin{itemize}
  \item \textbf{Predictability}: Users and developers can rely on the system's core behavior not being changed by governance
  \item \textbf{Immutability}: Key economic parameters are as unmodifiable as Bitcoin; battle-tested contracts can be permanently frozen
  \item \textbf{Minimal Trust}: Reduces trust assumptions about governors, lowers attack surface; frozen contracts require zero trust
  \item \textbf{Safe Iteration}: UUPS proxy pattern enables bug fixes during the early maturation period without disrupting user-facing addresses or stored state
  \item \textbf{Emergency Flexibility}: Retains minimum necessary adjustment capability for extreme situations
  \item \textbf{Credible Path}: Progressive decentralization provides a verifiable, irreversible path from admin control to community governance to permanent immutability
\end{itemize}
