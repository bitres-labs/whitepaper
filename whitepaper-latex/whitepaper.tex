...
\author{Invariant Core\\\texttt{invariantcore@gmail.com}\\\url{https://github.com/bitres-labs}}
\date{\today}
...
There are two important interest rates in the Bitres: deposit rate and bond rate. All interest rates described in this section are algorithmic policy variables rather than contractual guarantees. The deposit rate refers to the interest rate for users staking their BTD in the system, similar to bank deposits. The bond rate refers to the interest rate for users staking their BTB in the system, where users can earn staking interest over time.

\subsection{Deposit Rate}

The system's initial deposit rate is set to the Federal Reserve's federal funds rate at deployment. Thereafter, the deposit rate will be dynamically adjusted according to the system's collateral ratio to maintain system stability and attractiveness. The adjustment strategy is as follows:

When the collateral ratio is low, the system will increase the deposit rate to attract more users to deposit BTC to mint more BTD, thereby lowering the cost of BTC in the treasury. Conversely, when the collateral ratio is high, the system will decrease the deposit rate to encourage users to burn BTD to redeem BTC, thereby reducing the proportion of high-cost BTC in the treasury. Adjusting the deposit rate according to the collateral ratio level is essentially the BRS system performing counter-cyclical operations, that is, attracting more BTC into the treasury during bear markets and encouraging users to redeem BTC during bull markets, thereby achieving dynamic adjustment of BTD supply and maintaining the 1 BTD = 1 IUSD peg.
