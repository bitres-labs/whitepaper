...
The deposit rate is determined algorithmically based on BTD's market price relative to IUSD, implementing a negative feedback mechanism to maintain price stability. The principle is simple: when BTD's price falls below the anchor, the system raises the deposit rate to make holding BTD more attractive, thereby increasing demand and pushing the price back up; when BTD's price rises above the anchor, the system lowers the deposit rate to reduce the incentive for holding BTD. The rate adjusts continuously with price deviation---the further the price deviates from the anchor, the stronger the rate adjustment.
...
\begin{tabular}{c|c}
\toprule
BTD Price Range & Rate Policy \\
\midrule
$<$1.0 IUSD & Higher Rate (increases as price falls) \\
$=$1.0 IUSD & Base Rate \\
$>$1.0 IUSD & Lower Rate (decreases as price rises) \\
\bottomrule
\end{tabular}
\end{table}
...
The bond rate follows the same negative feedback principle as the deposit rate, but targets BTB's price relative to BTD. When BTB's price falls below the anchor (1 BTD), the system raises the bond rate to increase attractiveness; when BTB's price rises above the anchor, the system lowers the bond rate. Additionally, the bond base rate is adjusted according to the collateral ratio---when the collateral ratio is below 100\%, the base rate increases to compensate for the additional risk that BTB cannot be immediately redeemed.
...
\begin{tabular}{c|c}
\toprule
BTB Price Range & Rate Policy \\
\midrule
$<$1.0 BTD & Higher Rate (increases as price falls) \\
$=$1.0 BTD & Base Rate \\
$>$1.0 BTD & Lower Rate (decreases as price rises) \\
\bottomrule
\end{tabular}
\end{table}
